...
function App() {
  ...
  const connectTwitch = useCallback(() => {
    if (!twitchEnabled || !twitchToken || !twitchUsername) {
      return;
    }
    ...
    ws.onopen = () => {
      ...
      const token = twitchToken.startsWith("oauth:") ? twitchToken : `oauth:${twitchToken}`;
      ws.send(`PASS ${token}`);
      ws.send(`NICK ${twitchUsername.toLowerCase()}`);
  ...
  }, [twitchEnabled, twitchToken, twitchUsername, addMessage]);

  // Connect/disconnect Twitch when enabled changes
  useEffect(() => {
    if (twitchEnabled && twitchToken && twitchUsername) {
      connectTwitch();
    } else if (!twitchEnabled && twitchWsRef.current) {
      twitchWsRef.current.close();
      setTwitchStatus("Disabled");
      setTwitchConnected(false);
    }
  ...
  }, [twitchEnabled, twitchToken, twitchUsername, connectTwitch]);
      ...
      <div className="main">
        ...
        {activeTab === "tab-overlay" && (
          <OverlayTab 
            ...
            setKickChatroomId={setKickChatroomId}
  ...
  const generateShortUrl = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Use API endpoint that serves HTML with proper Lightstream headers
        const url = `${window.location.origin}/api/stream/${data.id}`;
        setShortUrl(url);
      } else {
